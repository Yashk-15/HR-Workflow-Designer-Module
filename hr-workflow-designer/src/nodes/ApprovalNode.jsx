...
function ApprovalNode({ data, selected }) {
  ...
  return (
    <div className={`relative min-w-[180px] rounded-xl shadow-lg border-2 transition-all duration-200
      ...
      bg-linear-to-br from-amber-950 to-amber-900`}>
      <Handle type="target" position={Position.Top} className="bg-amber-400! border-amber-700! w-3! h-3!" />
      ...
      <div className="px-3 py-2 space-y-0.5">
        <p className="text-sm font-medium text-white truncate max-w-[160px]">{data?.label || 'Untitled Approval'}</p>
        {data?.approverRole           && <p className="text-xs text-amber-300/70 truncate">🎯 {data.approverRole}</p>}
        {data?.autoApproveThreshold != null && <p className="text-xs text-amber-300/50">Auto @{data.autoApproveThreshold}%</p>}
      </div>
      <Handle type="source" position={Position.Bottom} className="bg-amber-400! border-amber-700! w-3! h-3!" />
    </div>
  );
}
